...
def tif2cog(input_paths: List[Path], type: str, value_type: str="category") -> None:
    for input_path in input_paths:
        output_cog_path = input_path.with_suffix(".cog.tif")
        ...
        if (type=='raw'): 
            ...
            cog_command= "gdalwarp -of COG -co COMPRESS=LZW -co BIGTIFF=YES -co NUM_THREADS=ALL_CPUS -co OVERVIEWS=IGNORE_EXISTING -wo NUM_THREADS=4 -multi  %s %s" % (temp_vrt_path, output_cog_path)
        elif (type=='display'):
...
#"gdalwarp -of COG -co COMPRESS=DEFLATE -co NUM_THREADS=ALL_CPUS -co OVERVIEWS=IGNORE_EXISTING -wo NUM_THREADS=4 -multi  %s %s" % (temp_vrt_path, output_cog_path)